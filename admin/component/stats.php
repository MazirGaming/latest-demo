	...
	function results() {
		...
		switch ($range) {
			...
			case 'day':
				...
				$results = $this->getTotalUsersByDay();

				foreach ($results as $key => $value) {
					$json['user']['data'][] = [$key, $value['total']];
				}
			...
			case 'week':
				...
				$results = $this->getTotalUsersByWeek();
			...
			case 'month':
				...
				$results = $this->getTotalUsersByMonth();
			...
			case 'year':
				...
				$results = $this->getTotalUsersByYear();
