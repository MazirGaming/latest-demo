...
class Plugins extends Base {
	...
	function index() {
		...
		$view->plugins    = PluginsList :: getList($this->global['site_id'], $view->category);
		...
		foreach ($files as $file) {
			...
			if (! $error) {
				if ($this->pluginSlug) {
					...
					$successMessage .= "<p><a class='btn btn-primary btn-sm m-2'  href='$pluginActivateUrl'>" . __('Activate plugin') . '</a></p>';
		...
		if ($active) {
			...
			if (defined('CLI')) {
				...
				$response   = "
					...
					<script>
					function reloadPage() {
						parent.location='$refreshUrl&success=$success';
					}
					</script>
					</head>
			...
			die(__('Error activating plugin!<br/>Check config file permissions!'));
		}
