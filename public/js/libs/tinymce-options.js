...
var wrapNodes = ['B','I','SPAN', 'TABLE', 'P'];
...
if (window.matchMedia("(prefers-color-scheme: dark)").matches || localStorage.getItem("theme", "dark") == "dark") {
	 ...
	 editor.ui.registry.addButton('quickmedia', {
        ...
        onAction: function () {
			...
			Vvveb.MediaModal.open(null, function (file) {
				editor.insertContent("<div>" + editor.dom.createHTML('img', { src: file, "class": "align-center" }) + "</div>");
			});
			editor.fire("blur");
			//console.log(editor);
        }
