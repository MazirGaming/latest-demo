...
class InsertScriptsPlugin {
	...
	function app() {
		...
		$options = Vvveb\get_setting(['insert-scripts.header', 'insert-scripts.footer']);
		$view->plugins += $options ?? [];
		$template->loadTemplateFile(__DIR__ . '/app/template/common.pst');

		/*
		$admin_path = \Vvveb\adminPath();
		Event::on('admin-bar', 'top', __CLASS__, function ($menu) use ($admin_path) {
			$menu['insert-scripts'] = [
				'name'     => __('Insert scripts'),
				'url'      => $admin_path . '?module=plugins/insert-scripts/settings',
				'icon-img' => PUBLIC_PATH . 'plugins/insert-scripts/insert-scripts.svg',
