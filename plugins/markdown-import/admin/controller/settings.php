		...
		foreach ($cats as $category_slug) {
			...
			if ($cat) {
				...
				$cat = $this->categories->addCategory([
					...
					'taxonomy_item_content' => $this->global + ['slug'=> $category_slug, 'name' => \Vvveb\humanReadable($category_slug), 'description' => \Vvveb\humanReadable($category_slug)],
				] + $this->global);
		...
		if (! $post_data) {
			$data =
				[
					'post' => $this->global + $params + ['post_content' => [
						1=> $params + ['language_id' => 1, 'name' => $name, 'slug' => $slug, 'content' => $html, 'excerpt' => ''],
					]],
				] + $this->global;
			...
			$data = $this->global + [
				'post' => $params + ['post_content' => [
					1=> $params + ['language_id' => 1, 'name' => $name, 'slug' => $slug, 'content' => $html],
				]],
