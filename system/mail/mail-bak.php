...
class Mail {
	...
	public function message() {
		if (! $this->option['html) {
			...
			if ($this->option['text) {
				...
				$message .= base64_encode(strip_tags($this->option['html)) . EOL;
			}

			$message .= $this->option['attachments();
	...
	public function send() {
		...
		if ($this->option['parameter) {
			return mail($to, $subject, $this->option['message, $this->option['header, $this->option['parameter);
		} else {
			return mail($to, $subject, $this->option['message, $this->option['header);
		}
	}
}
