...
function get_setting($key = null, $default = null, $site_id = SITE_ID) {
	return System\Setting::getInstance()->get($key, $default);
}

function set_setting($key = null, $value = null, $site_id = SITE_ID) {
	return System\Setting::getInstance()->set($key, $value);
}

function set_settings($options, $site_id = SITE_ID) {
	return System\Setting::getInstance()->multiSet($options);
}
...
function getTemplateList($theme = null, $skip = []) {
	...
	foreach ($files as $file) {
		...
		$pages[$name]  = ['name' => $name, 'filename' => $filename, 'file' => $file, 'url' => $url, 'title' => ucfirst($title), 'folder' => $path, 'description' => $description];
...
function model($model) {
	$modelClass = 'Vvveb\Sql\\' . $model . 'SQL';
 ...
 * @return bool 
 */
function email($to, $subject, $template, $data = []) {
	$email = System\Email :: getInstance();
	$view  = new System\Core\View();
	$view->setTheme();
	$view->set($data);
	$view->template("email/$template.html");

	$html = $view->render(true, false, true);

	//return $html;
	//return  file_put_contents(DIR_ROOT . 'mail.html', $html);
	$email->setHtml($html);
	$email->setTo($to);
	$email->setFrom('test@vveb.com');
	$email->setSender('admin@vvveb.com');
