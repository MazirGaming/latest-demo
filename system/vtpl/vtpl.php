		...
		for ($i = 0; $i < $length; $i++) {
			...
			if ($node && $node->nodeType == XML_TEXT_NODE &&
				...
				if (trim($text) != '') {
					...
					$php  = '<_script language="php"><![CDATA[ echo \Vvveb\__(\'' . trim($text) . '\');]]></_script>';
		...
		*/
		...
		if ($this->checkSyntax) {
			...
			try {
				...
				return \Vvveb\System\Core\FrontController :: notFound(false, 500, $data);
			}
		} else {
			file_put_contents($compiledFile, $html);
		}
