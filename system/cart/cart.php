...
class Cart {
	...
	private function updateCart() {
		...
		foreach ($results['products'] as $id => &$product) {
			...
			$product['quantity']        = $this->products[$productId]['quantity'] ?? 1;
	...
	public function addTax($name, $price, $taxClassId) {
		if ($price && $taxClassId) {
			$this->taxes[$taxClassId] = ['price' => $price, 'tax_class_id' => $taxClassId];
		}
	}
	...
	*/
	...
	public function write() {
		...
		foreach (['products', 'totals', 'total_items'] as $property) {
